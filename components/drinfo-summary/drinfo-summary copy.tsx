...
export function DrInfoSummary({ user, chatId, onChatCreated }: DrInfoSummaryProps) {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    // Use the direct content version to avoid state issues
    handleSearchWithContent(query);
  }
              ...
              <div className="flex-1 overflow-auto mb-4 max-w-3xl mx-auto w-full font-sans overflow-visible" ref={contentRef}>
                ...
                <div className="space-y-8">
                  {chatHistory.map((message) => {
                    console.log(`[RENDER] Rendering message ${message.id}, type: ${message.type}`);
                    if (message.type === 'assistant') {
                      console.log(`[RENDER] Assistant message has answer: ${Boolean(message.answer)}, main summary length: ${message.answer?.mainSummary?.length || 0}`);
                    }
                          ...
                          <div className="mb-8">
                                  ...
                                  <div 
                                    ...
                                    dangerouslySetInnerHTML={{ 
                                      __html: formatWithCitations(
                                        message.answer?.mainSummary || message.content || '', 
                            ...
                            {message.answer?.sections && message.answer.sections.length > 0 && (
                              <div className="space-y-6 ml-8">
                                {message.answer.sections.map((section) => (
                                  <div key={section.id}>
                            ...
                            {message.answer?.citations && Object.keys(message.answer.citations).length > 0 && (
                              <div className="mt-6 ml-8">
                                <p className="text-slate-500 text-sm">
                                  Used {getCitationCount(message.answer.citations)} references
                                </p>
                                <ReferenceGrid
                            ...
                            {user && currentChatId && (
                              <AnswerFeedback
                                ...
                                onFeedbackUpdate={(newFeedback) => handleFeedbackUpdate(message.id, newFeedback)}
                                answerText={message.answer?.mainSummary || message.content || ''}
                              />
                            )}
                          </div>
                        )}
